...
extern buffer** buf;
extern char** pathname_list;
extern int buffer_size;

extern pthread_mutex_t trx_sys_mutex, table_mutex, buff_mutex, log_mutex;

extern int next_trx;
extern queue<int> free_trx;
extern trx_t* trx;
extern unordered_map<int, LSH> table;

extern int trx_size;

int db_find(int table_id, int64_t key, char* ret_val, int trx_id)
{
	//buffer pool latch를 얻음
	pthread_mutex_lock(&buff_mutex);
	trx_t* t = &trx[trx_id];
	int pgnum = find_leaf(table_id, key);
	page_t leaf;
	file_read_page(table_id, pgnum, &leaf);
	int idx = get_buffer_index(table_id, pgnum);
	if (idx == -1)
	{
	...
	(tmp_buf->is_pinned)++;
	//page latch를 얻은 뒤 buffer pool latch를 해제함
	pthread_mutex_lock(&(tmp_buf->page_mutex));
	pthread_mutex_unlock(&buff_mutex);
	file_read_page(table_id, pgnum, &leaf);
	//chk가 0이면 실행, 1이면 wait, 2이면 deadlock이 발생하여 abort시켜준다.
	int chk = acquire_record_lock(table_id, 0, key, trx_id);
	//deadlock이 발생하지 않을 때
	if (chk == 0 || chk == 1)
	{
		//wait 해야할 때
		if (chk == 1)
		{
			pthread_cond_wait(&(t->trx_cond), &(t->trx_mutex));
			//wait이 끝난 후 wait_lock을 trx_locks에 추가시켜준 뒤 NULL로 바꿔줌
			trx[trx_id].trx_locks.push_back(trx[trx_id].wait_lock);
			trx[trx_id].wait_lock = NULL;
		}
		...
		if (idx != -1)
			strcpy(ret_val, leaf.record[idx].value);
		else
			ret_val = NULL;
		(tmp_buf->is_pinned)--;
...
int db_update(int table_id, int64_t key, char* values, int trx_id)
{
	...
	pthread_mutex_unlock(&buff_mutex);
	file_read_page(table_id, pgnum, &leaf);
	int chk = acquire_record_lock(table_id, 1, key, trx_id);
	if (chk == 0 || chk == 1)
	{
		if (chk == 1)
		{
		...
		if (idx != -1)
		{
			...
			tmp_log.trx_id = trx_id;
			tmp_log.type = 1;
			tmp_log.table_id = table_id;
			tmp_log.pgnum = pgnum;
			tmp_log.offset = idx;
			strcpy(tmp_log.old_img, leaf.record[idx].value);
			strcpy(tmp_log.new_img, values);
			...
			undo u;
			u.key = key;
			strcpy(u.value, leaf.record[idx].value);
			u.table_id = table_id;
			t->undo_list.push_back(u);
			strcpy(leaf.record[idx].value, values);
			file_write_page(table_id, pgnum, &leaf);
		}
...
int init_db(int num_buf)
{
	...
	for (int i = 1;i <= 10;i++)
	{
		...
		log_buf = (log_buffer**)malloc(sizeof(log_buffer*)*num_buf);
